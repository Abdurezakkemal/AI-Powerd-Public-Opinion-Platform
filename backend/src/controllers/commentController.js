...
exports.postComment = async (req, res) => {
  try {
    ...
    logger.info(
      `[POST_COMMENT] User ${req.user.id} posting comment to policy ${policyId}`,
    );

    if (!policyId || !text?.trim()) {
      logger.warn(
        `[POST_COMMENT] Validation failed - missing policyId or text`,
      );
      return sendError(
        res,
    ...
    logger.info(
      `[POST_COMMENT] Comment created successfully: ${comment._id} by user ${req.user.id}`,
    );
...
exports.getPolicyComments = async (req, res) => {
  try {
    ...
    logger.info(
      `[GET_COMMENTS] Request for policyId: ${policyId}, page: ${page}, limit: ${limit}`,
    );
    ...
    logger.info(
      `[GET_COMMENTS] policyId type: ${typeof policyId}, value: ${policyId}`,
    );
    ...
    logger.info(
      `[GET_COMMENTS] Executing aggregation pipeline with filter:`,
    ...
    logger.info(
      `[GET_COMMENTS] Raw comments from aggregation: ${comments.length}`,
    );
    ...
    logger.info(
      `[GET_COMMENTS] Sending response with ${formatted.length} comments, total: ${response.pagination.total}`,
    );
